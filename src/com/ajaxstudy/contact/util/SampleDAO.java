...
public class SampleDAO {
	...
	private static String nameList = "고수,공유,권상우,권율,김범,김수현," + "김우빈,현빈,남주혁,마동석,박보검,박서준," + "박해일,박해진,박형석,서강준,서인국,소지섭,"
			+ "송강호,신구,안재욱,양택조,옥택연,원기준," + "원빈,유오성,유지태,윤계상,윤상현,이계인," + "이광기,이덕화,이동욱,이병헌,이원종,이준기,"
			+ "인교진,임창정,임채무,전무승,전국환,전노민," + "정우성,정보석,정웅인,조인성,조정석,조진웅," + "주지훈,지성,지창욱,지진희,지현우,주원,"
			+ "차인표,차승원,최다니엘,최민수,최민식,최불암," + "하석진,현빈,홍광호,황정민";
	// 연락처 목록을 담을 변수 선언
	...
	public static ContactList getContacts() {
		...
		// (현재페이지0, 페이지사이즈0, 목록사이즈contacts.size(), List<Contact> contacts)
		ContactList cList = new ContactList(0, 0, contacts.size(), contacts);
		return cList;
	}

	// 특정 페이지와 특정 페이지에 포함될 연락처 개수 요청(일부 연락처 정보만 전송)
	public static ContactList getContacts(int pageno, int pagesize) {
		// 전송할 연락처 목록의 시작위치 지정
		int starteIndex = (pageno - 1) * pagesize;
		// 전송할 연락처 목록의 끝 위치 지정
		int endIndex = starteIndex + pagesize;

		List<Contact> temps = null;

		// 연락처 목록의 범위를 벗어난 요청이 오면
		if (starteIndex > contacts.size() - 1 || starteIndex < 0 || pagesize < 1) {
			// 빈 객체를 리턴
			...
			temps = contacts.subList(starteIndex, endIndex);
		}
		// 위에서 추출한 연락ㅊ 목록의 내용으로 된 연락처 목록 객체 생성
		// (현재페이지pageno, 페이지사이즈pagesize, 목록사이즈contacts.size(), List<Contact> temps)
	...
	public static int updateContact(Contact c) {
		...
		for (int i = 0; i < contacts.size(); i++) {
			...
			if (con.getNo() == no) {
				// 기존 연락처 정보를 매개변수로 받은 연락처 정보를 수정한다.
				contacts.set(i, c);
				count++;
				break;
			}
